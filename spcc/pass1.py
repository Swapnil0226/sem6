    ...
    if(len(s) == 2):
        ...
        for item in Mlist:
            if(item.mnemonic == s[0]):
                Mlist1.append(
                    MOT(s[0], item.binaryop, item.insLength, item.insFormat))
    else:
        if(len(s) == 3):
            if(s[1] == "START"):
        ...
        for i in range(len(s)):
            ...
            for item in Mlist:
                if(item.mnemonic == s[i]):
                    Mlist1.append(
                        MOT(s[i], item.binaryop, item.insLength, item.insFormat))
print()
print("SYMBOL TABLE : \n")
for item in STlist:
    print(item.symbol+"\t\t"+item.value+"\t\t" +
          str(item.length)+"\t\t"+item.relocation)
print("\n")
